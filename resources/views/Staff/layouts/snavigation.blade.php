<aside x-data="{ open: false }"
    ...
    <div class="flex-1">
        ...
        <div class="h-[85px] flex items-center px-6 border-b">
            <span class="text-2xl font-semibold">Staff-Menu</span>
        </div>
        ...
        <nav class="flex flex-col  space-y-4 px-4 py-4">
            ...
            <x-nav-link
                ...
                class="block px-3 text-[1.06rem]  py-2 rounded-md">
                {{ __('Dashboard') }}
            </x-nav-link>

            @can('view-package')
                <x-nav-link
                    :href="route('staffpackages.index')"
                    :active="request()->routeIs('staffpackages.index')"
            ...
            @can('view-customer')
                <x-nav-link
                    :href="route('staffcustomers.index')"
                    :active="request()->routeIs('staffcustomers.index')"
                    class="block px-3 py-2 text-[1.06rem]  rounded-md">
                    {{ __('Customers') }}
                </x-nav-link>
            @endcan
            ...
            @can('view-booking')
                <x-nav-link
                    :href="route('staffbookings.index')"
                    :active="request()->routeIs('staffbookings.index')"
                    class="block px-3 py-2 text-[1.06rem]  rounded-md">
                    {{ __('Bookings') }}
                </x-nav-link>
            @endcan
            ...
            <x-slot name="trigger">
                <button class="w-full flex items-center justify-between rounded-md px-3 py-2 hover:bg-gray-100 transition">
                    ...
                    <div class="flex items-center gap-3">
                        <!-- Avatar -->
                        <div class="h-9 w-9 flex items-center justify-center rounded-full bg-indigo-600 text-white font-semibold">
