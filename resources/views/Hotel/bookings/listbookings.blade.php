<x-hotel-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('hotelbookings.create') }}" class="bg-slate-700 text-sm rounded-md px-3 py-2 text-white">Add Booking</a>
        </div>
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="border-b">
                        ...
                        <td class="px-6 py-2 flex items-center  justify-center">
                            <a href="{{ route('hotelbookings.edit',$booking->id) }}" class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-600">Edit</a>

                            <form action="{{ route('hotelbookings.destroy', $booking->id) }}" method="POST" >
    @csrf
    @method('DELETE')
