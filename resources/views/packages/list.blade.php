{{-- <x-app-layout>
    <x-slot name="header">
        ...
        @can ('create packages')
        <a href="{{ route('packages.create') }}" class="bg-slate-700 text-sm rounded-md px-3 py-2 text-white">Add Package</a>
        </div>
        @endcan
    </x-slot>
          ...
          <x-message></x-message>
             <table class="w-full">
                ...
                <tbody class="bg-white">
                    ...
                    <tr class="border-b">
                        <td class="px-6 py-2 text-left w-10">{{$packages->firstItem()+$idx}}</td>
                        <td class="px-6 py-2 text-left ">{{$package->package_name}}</td>
                        <td  class="px-6 py-2 text-left">{{$package->hotel->name}}</td>
                        ...
                        <td class="px-6 py-2 flex items-center  justify-center">
                            <a href="{{ route('packages.edit',$package->id) }}" class="bg-slate-700 text-sm rounded-md text-white px-3 py-2 hover:bg-slate-600">Edit</a>
